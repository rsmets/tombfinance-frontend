...
import { FTM_TICKER, TOMB_TICKER, TRACKS_TICKER, TSHARE_TICKER, VINYL_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  else if (zappingToken === VINYL_TICKER) token = tombFinance.VINYL;
  else if (zappingToken === TRACKS_TICKER) token = tombFinance.TRACKS;
